...
def main(background_filename, dartboard_filename):
    background_image = cv2.imread(background_filename)
    background_image_segment = cv2.cvtColor(background_image, cv2.COLOR_BGR2GRAY)
    ...
    isolated_dartboard, _ = extracting_roi(background_image_segment)
    ...
    result_img, _, locations = Get_locations(diff, forground_image, score_contour)
    ...
    print("Dart Score:", dart_score)

if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python script.py background_image_filename dartboard_image_filename")
    else:
        background_filename = sys.argv[1]
        dartboard_filename = sys.argv[2]
        main(background_filename, dartboard_filename)
